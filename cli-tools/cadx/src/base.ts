...
abstract class EthCommand extends Command {
  ...
  uint256ToDecimals(int: any): number {
    ...
    if (Number.isNaN(num)) {
      this.error(`Trying to convert uint256 to decimals but input is ${int}`);
    }
    if (!this.web3().utils.toBN(num).eq(this.web3().utils.toBN(int))) {
      this.error(`Loss of precision converting from ${int} to ${num}`);
    }
    return num / 10 ** config.token.decimals;
  }

  decimalsToUint256(dec: number) {
    dec = Number(dec);
    let int = dec * 10 ** config.token.decimals;
    if (int > Number.MAX_SAFE_INTEGER) {
      this.error(
        `Trying to convert decimal value ${int} exceed Number.MAX_SAFE_INTEGER`
      );
    }
    int = Math.floor(int);
    return this.web3().utils.toBN(int);
  }
