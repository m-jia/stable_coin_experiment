...
contract('Issuer implementation switch', function ([_, owner, anotherAccount, implementation1, implementation2]) {
  beforeEach(async function () {
    const issuer = await CADXIssuer.new(implementation1, 3, {from: owner});
    this.issuer = issuer;
  });
  ...
  describe('when the caller is not the owner', function () {
    it('cannot switch the implementation', async function() {
      await assertRevert(this.issuer.setImplementation(implementation2, {from: anotherAccount}));
    });
  });
});
