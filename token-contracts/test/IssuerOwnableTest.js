const CADXIssuer = artifacts.require('CADXIssuer.sol');

const assertRevert = require('./helpers/assertRevert');
const {ZERO_ADDRESS} = require('openzeppelin-test-helpers').constants;

// Test that the Issuer operates correctly as Ownable.
contract('Issuer own', function ([_, owner, anotherAccount, anotherAccount2, randomAddress]) {
  beforeEach(async function () {
    const issuer = await CADXIssuer.new(randomAddress, 10, {from: owner});
    this.issuer = issuer;
  });
  ...
  describe('as an ownable', function () {
    ...
    it('sets new potential owner after proposeOwner', async function () {
      const {logs} = await this.issuer.proposeOwner(anotherAccount, {from: owner});
      let currentOwner = await this.issuer.owner();
      assert.equal(currentOwner, owner);
    ...
    it('removes proposed owner after disregardProposeOwner called by owner', async function () {
    	...
    	const {logs} = await this.issuer.disregardProposeOwner({from: owner});
    	let currentOwner = await this.issuer.owner();
    	assert.equal(currentOwner, owner);
    ...
    it('removes proposed owner after disregardProposeOwner called by proposed owner', async function () {
    	await this.issuer.proposeOwner(anotherAccount, {from: owner});
    	const {logs} = await this.issuer.disregardProposeOwner({from: anotherAccount});
    	let currentOwner = await this.issuer.owner();
    	assert.equal(currentOwner, owner);
    ...
    it('should prevent non-proposed owners from calling disregardProposeOwner', async function () {
      ...
      await assertRevert(this.issuer.disregardProposeOwner({from: anotherAccount2}));
    });
    ...
    it('sets new owner after completeTransferOwnership', async function () {
      await this.issuer.proposeOwner(anotherAccount, {from: owner});
      const {logs} = await this.issuer.claimOwnership({from: anotherAccount});
      let currentOwner = await this.issuer.owner();
    ...
    it('should prevent non-owners from transferring ownership', async function () {
      ...
      await assertRevert(this.issuer.proposeOwner(anotherAccount2, {from: anotherAccount2}));
    });
    ...
    it('should prevent non-proposedOwners from finishing transferring ownership', async function () {
      ...
      await assertRevert(this.issuer.claimOwnership({from: anotherAccount2}));
    });

    it('should guard ownership against stuck state', async function () {
      let originalOwner = await this.issuer.owner();
      await assertRevert(this.issuer.proposeOwner(ZERO_ADDRESS, {from: originalOwner}));
    });
  });
});
