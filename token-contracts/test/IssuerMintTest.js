...
contract('Issuer mint', function ([_, admin, owner, member1, member2, notmember]) {
  ...
  beforeEach(async function () {
    ...
    const issuer = await CADXIssuer.new(proxiedCADX.address, mintWaitBlocks, {from: owner});
    this.issuer = issuer;
  ...
  describe('when the caller is a member', function () {
    it('proposes new mint', async function() {
      const {logs} = await this.issuer.proposeMint(notmember, 100, {from: member1});
      ...
      assert.equal(pendingMint.canMintAtBlock, latestBlock.number + mintWaitBlocks);
    });
  ...
  describe('when the caller is not a member', function () {
    it('cannot propose new mint', async function() {
      await assertRevert(this.issuer.proposeMint(notmember, 100, {from: notmember}));
    });
