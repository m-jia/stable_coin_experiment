...
// Tests that CADX token issuance mechanisms operate correctly.
contract('CADX issue', function ([_, admin, newIssuer, otherAddress, owner]) {
  ...
  describe('as an issuable token', function () {
    ...
    describe('mintTo', function () {
      ...
      it('reverts when sender is not issuer', async function () {
        await assertRevert(this.token.mintTo(otherAddress, amount, {from: otherAddress}));
      });

      it('reverts when the recipient is the zero address', async function () {
        await assertRevert(this.token.mintTo(ZERO_ADDRESS, amount, {from: owner}));
      });
      ...
      it('emits a Mint and a Transfer event', async function () {
        const {logs} = await this.token.mintTo(otherAddress, amount, {from: owner});
    ...
    describe('setIssuer', function () {
      ...
      it('reverts if sender is not owner', async function () {
        await assertRevert(this.token.setIssuer(otherAddress, {from: newIssuer}));
      });
