const CADX1 = artifacts.require('CADXImplementation.sol');
const CADX2 = artifacts.require('CADXWithBalance.sol');
const Proxy = artifacts.require('CADXProxy.sol');

const assertRevert = require('./helpers/assertRevert');
const {ZERO_ADDRESS} = require('openzeppelin-test-helpers').constants;

// Test that CADX is upgradeable
contract('CADX upgrade', function ([_, admin, owner, issuer, compliance, anotherAccount]) {

  beforeEach(async function () {
    const implementation = await CADX1.new({from: owner});
    const proxy = await Proxy.new(implementation.address, admin, {from: admin});
    const proxiedCADX = await CADX1.at(proxy.address);
  ...
  describe('as an admin', function () {
    ...
    it ('upgrades the implementation address', async function () {
      const newImplementation = await CADX2.new({from: owner});
      ...
      assert.equal(web3.utils.toChecksumAddress(newImplementationAddress), newImplementation.address);
    });

    it ('sets new admin', async function () {
      const {logs} = await this.proxy.changeAdmin(anotherAccount, {from: admin});
  ...
  describe('as a non-admin', function () {
    it ('can call non-admin functions', async function () {
      const totalSupply = await this.token.totalSupply({from: anotherAccount});
    ...
    it ('cannot upgrade the implementation address', async function () {
      const newImplementation = await CADX2.new({from: owner});
      await assertRevert(this.proxy.upgradeTo(newImplementation.address, {from: anotherAccount}));
    });

    it ('cannot set new admin', async function () {
      await assertRevert(this.proxy.changeAdmin(anotherAccount, {from: anotherAccount}));
    });
  });
  ...
  describe('as a new implementation', function () {
    ...
    beforeEach(async function () {
      ...
      const newImplementation = await CADX2.new({from: owner});
      await this.proxy.upgradeTo(newImplementation.address, {from: admin});
      const proxiedCADX = await CADX2.at(this.proxy.address);
      this.newToken = proxiedCADX;
    });
    ...
    it ('can call new functions', async function () {
      await this.newToken.topupBalance(anotherAccount, 888, {from: anotherAccount});
