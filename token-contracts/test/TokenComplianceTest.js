const CADXContract = artifacts.require('CADXImplementation.sol');
const Proxy = artifacts.require('CADXProxy.sol');

const assertRevert = require('./helpers/assertRevert');
const {ZERO_ADDRESS} = require('openzeppelin-test-helpers').constants;

// Tests that CADX compliance capabilities function correctly.
contract('CADX regulatory compliance', function ([_, admin, complianceRole, otherAddress, freezableAddress, owner]) {

  beforeEach(async function () {
    const CADX = await CADXContract.new({from: owner});
    const proxy = await Proxy.new(CADX.address, admin, {from: admin});
    const proxiedCADX = await CADXContract.at(proxy.address);
  ...
  describe('as a regulatory compliant token', function () {
    ...
    describe('freeze', function () {
      ...
      it('adds the frozen address', async function () {
        ...
        const frozen = await this.token.frozen(freezableAddress, {from: complianceRole});
        assert.equal(frozen, true, 'address is frozen');
      });

      it('emits a FreezeAddress event', async function () {
        const {logs} = await this.token.freeze(freezableAddress, {from: complianceRole});
      ...
      describe('when frozen', function () {
        ...
        it('reverts when mintTo is to frozen address', async function () {
          await assertRevert(this.token.mintTo(freezableAddress, approvalAmount, {from: owner}));
        });

        it('reverts when burnFrom targets frozen address', async function () {
          await assertRevert(this.token.burnFrom(freezableAddress, approvalAmount, {from: owner}));
        });

        it('reverts when burnFrom called by frozen address', async function () {
          await assertRevert(this.token.burnFrom(owner, approvalAmount, {from: freezableAddress}));
        });
      });

      it('reverts when address is already frozen', async function () {
        await this.token.freeze(freezableAddress, {from: complianceRole});
        await assertRevert(this.token.freeze(freezableAddress, {from: complianceRole}));
      });
    });

    describe('unfreeze', function () {
      it('reverts when address is already unfrozen', async function () {
        await assertRevert(this.token.unfreeze(freezableAddress, {from: complianceRole}));
      });
      ...
      describe('when already frozen', function () {
        ...
        it('removes a frozen address', async function () {
          await this.token.unfreeze(freezableAddress, {from: complianceRole});
        ...
        it('emits an FreezeAddress event', async function () {
          const {logs} = await this.token.unfreeze(freezableAddress, {from: complianceRole});
    ...
    describe('wipeFrozenAddress', function () {
      it('reverts when address is not frozen', async function () {
        await assertRevert(this.token.wipeFrozenAddress(freezableAddress, {from: complianceRole}));
      });
      ...
      describe('when already frozen with assets and approvals', function () {
        ...
        it('wipes a frozen address balance', async function () {
          await this.token.wipeFrozenAddress(freezableAddress, {from: complianceRole});
        ...
        it('emits an WipeFrozenAddress event', async function () {
          const {logs} = await this.token.wipeFrozenAddress(freezableAddress, {from: complianceRole});
    ...
    describe('setComplianceRole', function () {
      ...
      it('reverts if sender is not owner or ComplianceRole', async function () {
        await assertRevert(this.token.setComplianceRole(otherAddress, {from: otherAddress}));
      });

      it('works if sender is ComplianceRole', async function () {
        await this.token.setComplianceRole(otherAddress, {from: complianceRole});
      ...
      it('emits a ComplianceRoleSet event', async function () {
        const {logs} = await this.token.setComplianceRole(otherAddress, {from: complianceRole});
