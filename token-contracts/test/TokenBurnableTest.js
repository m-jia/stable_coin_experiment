...
// Tests that the CADX token burn mechanism operates correctly
contract('CADX burn', function ([_, admin, newIssuer, otherAddress, owner]) {
    ...
    describe('burn', function () {
      ...
      describe('when the burner has sufficient tokens', function () {
        ...
        it('emits a Burn and a Transfer event', async function () {
          const {logs} = await this.token.burn(decreaseAmount, {from: owner});
    ...
    describe('burnFrom', function () {
      ...
      describe('when the target has enough approved balance', function () {
        ...
        describe('when the target has insufficient tokens', function () {
          it('reverts', async function () {
            await assertRevert(this.token.burnFrom(owner, decreaseAmount, {from: otherAddress}));
          });
        });
        ...
        describe('when the target has sufficient tokens', function () {
          ...
          it('removes the requested amount', async function () {
            await this.token.burnFrom(owner, decreaseAmount, {from: otherAddress});
          ...
          it('emits a Burn and a Transfer event', async function () {
            const {logs} = await this.token.burnFrom(owner, decreaseAmount, {from: otherAddress});
