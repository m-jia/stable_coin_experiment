...
contract('Issuer membership', function ([_, admin, owner, member1, member2, notmember]) {
  beforeEach(async function () {
    ...
    const issuer = await CADXIssuer.new(proxiedCADX.address, 3, {from: owner});
    this.issuer = issuer;
  });
  ...
  describe('when the caller is the owner', function () {
    ...
    it('removes existing member', async function() {
      ...
      const {logs} = await this.issuer.removeMember(member2, {from: owner});
      // Check events emitted
      assert.equal(logs.length, 1);
    ...
    it('cannot add members beyond limit', async function() {
      ...
      await Promise.all(
        [...Array(255).keys()].map(i => this.issuer.addMember(addresses[i], {from: owner}))
      );
