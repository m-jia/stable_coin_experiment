...
/**
 * @title CADXIssuer
 * @dev This contract will be the issuer for CADX. It provides a check and balance on
...
contract CADXIssuer {
    ...
    // IMPLEMENTATION
    CADXImplementation internal _implementation;
    ...
    event MintWaitBlocksUpdated(uint256 oldMintWaitBlocks, uint256 newMintWaitBlocks);
    ...
    constructor(address initialImplementation, uint256 initialMintWaitBlocks) public {
        _numMembers = 0;
    ...
    function setImplementation(address implementationAddress) public onlyOwner {
        emit ImplementationChange(address(_implementation), implementationAddress);
        _implementation = CADXImplementation(implementationAddress);
    }
    ...
    function setMintWaitBlocks(uint256 newMintWaitBlocks) public onlyOwner returns (bool) {
     ...
     */
    function proposeMint(address to, uint256 value) public onlyMember returns (uint256 index) {
        pendingMints[_pendingMintsIndex] = PendingMint(to, value, block.number + _mintWaitBlocks);
    ...
    function sendMint(uint256 index) public onlyMember returns (bool success) {
        require(block.number >= pendingMints[index].canMintAtBlock, "cannot send mint until sufficient blocks have passed");

        _implementation.mintTo(pendingMints[index].recipient, pendingMints[index].value);
     ...
     * @return A boolean that indicates if the operation was successful.
     */
    function rejectMint(uint256 index) public onlyMember returns (bool success) {
        require(pendingMints[index].value > 0, "this pending mint does not exist");
    ...
    function proposeOwner(address newProposedOwner) public onlyOwner {
        require(newProposedOwner != address(0), "cannot transfer ownership to address zero");
    ...
    function disregardProposeOwner() public {
        require(msg.sender == _proposedOwner || msg.sender == _owner, "only proposedOwner or owner");
        require(_proposedOwner != address(0), "can only disregard a proposed owner that was previously set");
